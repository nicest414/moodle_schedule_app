    ...
    if (myCourseCardList.length > 0) {
      ...
      for (const element of myCourseCardList) {
        ...
        myCourseDataList.push({
          ...
          year: parseInt(
            (element.querySelector(".categoryname") || 0).innerText || 0
          ),
        });
      }
...
function showLoadingScreen() {
  ...
  loadingScreen.innerText = "Loading..."; // テキストを表示
...
function hideLoadingScreen() {
  ...
  const allItem = document.querySelector('a[data-pref="inprogress"]');
...
function getMyCourseButtonClickHandler() {
  ...
  const display96Item = document.querySelector('a[data-limit="96"]');
  //const allItem = document.querySelector('a[data-pref="all"]');
...
function getCalendarURLButtonClickHandler() {
  ...
  getCalendarURL()
    .then((calendarUrl) => {
      // 非同期処理が完了したらローディング画面を削除
      popupButton.style.boxShadow = "0 4px 10px rgba(0, 0, 0, 0.4)";
      console.log(JSON.stringify({ calendarUrl: calendarUrl }));
      hideLoadingScreen();
    })
    .catch((error) => {
      // エラーが発生した場合もローディング画面を削除
...
function createDropdownMenu() {
  ...
  menuItem2.href = "#";
  menuItem2.innerText = "課題の自動取得を設定する";
...
function _getSessionKey() {
  ...
  if (logoutLink) {
    ...
    var sessKeyMatch = href.match(/sesskey=([^&]+)/);
    return sessKeyMatch ? sessKeyMatch[1] : "";
  }
  return "";
}
...
async function getCalendarURL() {
  const calendarExportUrl = "https://wsdmoodle.waseda.jp/calendar/export.php?";
  ...
  try {
    let response = await fetch(calendarExportUrl, {
      method: "POST",
      headers: headers,
      body: body,
    });
    ...
    if (response.ok) {
      ...
      const inputElement = doc.querySelector("#calendarexporturl");
      if (inputElement) {
        console.log(inputElement.getAttribute("value"));
        return inputElement.getAttribute("value") || "";
      } else {
        throw new Error("カレンダーURLが見つかりませんでした");
      }
    } else {
      throw new Error(`HTTPリクエストが失敗しました: ${response.status}`);
    }
  } catch (error) {
    throw new Error(`Fetch操作に問題が発生しました: ${error}`);
  }
}
